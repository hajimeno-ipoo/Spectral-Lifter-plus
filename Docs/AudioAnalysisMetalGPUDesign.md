...
現在の実験Metal解析では、`separatedMeanSpectra` のうち magnitude 計算だけをMetalで試作しています。`device`、`commandQueue`、`pipeline` はキャッシュし、中央値フィルタとharmonic / percussive集計はCPU側に残しています。

GPU版magnitudeの複数回・複数長ベンチと、解析値差分の専用テストは追加済みです。次に進めるなら、結果を見ながら集計部分をMetalへ寄せるか判断します。
