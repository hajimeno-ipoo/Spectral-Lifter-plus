...
GPU向きではあります。各フレーム、各ビンを独立して処理しやすいからです。`Spectrogram` は連続した1本のバッファになっているため、以前よりGPUへ渡しやすい状態です。
...
GPU化の候補ではありますが、現在はCPU側で全フレーム分の `foldReal` / `foldImag` を作らず、1フレームずつ逆FFTへ流す形に軽量化済みです。
...
| --- | --- | --- | --- | --- |
| `SpectralGateDenoiser` mask適用 | 中 | 中 | 中 | 試作候補 |
| `HarmonicUpscaler.foldover` | 低〜中 | 中 | 中 | CPU側の大きな一時配列削減は実施済み |
