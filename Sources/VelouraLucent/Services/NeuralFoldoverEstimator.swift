...
struct NeuralFoldoverEstimator {
    func predict(features: NeuralFoldoverFeatures) -> NeuralFoldoverPrediction {
        ...
        let hidden1 = relu(0.62 * harmonic + 0.74 * cutoffDeficit - 0.36 * shimmer - 0.28 * noise + 0.18 * transient - 0.12)
        let hidden2 = relu(0.55 * transient + 0.34 * cutoffDeficit - 0.18 * brightness - 0.22 * noise + 0.08)
        let hidden3 = relu(0.48 * shimmer + 0.41 * noise + 0.24 * brightness - 0.22 * harmonic + 0.06)

        let rawFoldover = 0.09 + hidden1 * 0.18 + hidden2 * 0.10 - hidden3 * 0.14
        let rawAirBias = 0.03 + hidden1 * 0.16 - hidden3 * 0.12 + (0.5 - brightness) * 0.06
        let rawTransientBias = 0.01 + hidden2 * 0.18 + transient * 0.05 - noise * 0.08
        let rawHarshnessGuard = 0.16 + hidden3 * 0.34 + shimmer * 0.22 + noise * 0.24 - harmonic * 0.08
