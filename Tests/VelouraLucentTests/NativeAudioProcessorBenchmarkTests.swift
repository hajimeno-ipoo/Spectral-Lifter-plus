...
struct NativeAudioProcessorBenchmarkTests {
    ...
    func recordsStageTimings() throws {
        ...
        #expect(logs.values.contains { $0.hasPrefix("ノイズ除去/10-16kHzチラつき: ") })
        #expect(logs.values.contains { $0.hasPrefix("ノイズ除去/12kHz以上: ") })
        #expect(logs.values.contains { $0.hasPrefix("ノイズ除去/16kHz以上: ") })
        #expect(logs.values.contains { $0.hasPrefix("ノイズ除去/18kHz以上: ") })
